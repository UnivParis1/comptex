...
import { addDays, inclusive_range as range, sameKeyNameChoices } from './helpers';
...
const _explain_main_profile = (v: v) => {
    ...
    if ((main_prof?.priority || 0) < 590 && v.global_supannEtuAnneeInscription && v.global_eduPersonPrimaryAffiliation === 'student') {
...
export const create_temp_profile_for_disabled_account = async (uid: string, structureParrain: string) => {
    ...
    const v: v = { uid, profilename: '{COMPTEX}reactiv', structureParrain, enddate: addDays(new Date(), 2) }
