...
export const may_create_temp_profiles_for_disabled_accounts = async (homonymes: v[]) => {
    for (const homo of homonymes) {
        ...
        if (homo.accountStatus === 'disabled') {
            await conf_up1.create_temp_profile_for_disabled_account(homo.uid, 'IU1')
        }
    }
}
...
export default ({ labels, attrs, action_pre, action_post, next, ...step_other }: Pick<step, "labels"|"attrs"|"action_pre_before_save"|"action_pre"|"action_post"> & { next?: string }): step => ({
    ..._.pick(step_other, "action_pre_before_save"),
    labels: {
        ...labels,
