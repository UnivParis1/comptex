...
export default ({ labels, attrs, action_pre, action_post, next, ...step_other }: Pick<step, "labels"|"attrs"|"action_pre_before_save"|"action_pre"|"action_post"|"action_before_purge"> & { next?: string }): step => ({
    ...
    labels: {
        ...labels,
        description: 
            labels.description?.match('{{{choices}}}') ?
                labels.description.replace('{{{choices}}}', our_description) :
            (labels.description ? `<div class="pre_description">${labels.description}</div>` : '') + our_description,
    },
