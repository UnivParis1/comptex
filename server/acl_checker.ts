...
// simple helper using "v_to_moderators_ldap_filter" to get LDAP people with a specific attribute
export const v_to_users = (acls: acl_search, v: v, attr: string) => (
    acls.v_to_moderators_ldap_filter(v).then(filter => searchPeople(filter, attr))
);
...
const v_to_moderators_ldap_filter = (v: v, acls: acl_search[]) => (
    Promise.all(acls.map(acl => acl.v_to_moderators_ldap_filter(v))).then(ldap_filters.or)
);

// Returns the ldap filter a "user" is allowed to moderate for "acls"
export const loggedUser_to_ldap_filter = async (user: CurrentUser, acls: acl_search[]) => {
    if (!acls) {
        return true;
    } else if (!user) {
        return false;
    } else {
        return Promise.all(acls.map(acl => acl.loggedUser_to_ldap_filter(user))).then(_simplify_ldap_filters)
    }
};

// Returns the mongo filter a "user" is allowed to moderate for "acls"
const loggedUser_to_mongo_filter = async (user: CurrentUser, acls: acl_search[]) => {
    if (!acls) {
        return true;
    } else if (!user) {
        return false;
    } else {
        return Promise.all(acls.map(acl => acl.loggedUser_to_mongo_filter(user))).then(_simplify_mongo_filters)
    }
};
...
const one_allowed_step_ldap_filter = (user: CurrentUser) => (step: step, stepName: string) => (
    loggedUser_to_ldap_filter(user, step.acls).then(filter => (
        { step: stepName, filter: filter }
    ))
);

const one_allowed_step_mongo_filter = (user: CurrentUser) => (step: step, stepName: string) => (
    loggedUser_to_mongo_filter(user, step.acls).then(filter => (
        { step: stepName, filter: filter }
    ))
);
