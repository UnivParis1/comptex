...
* ```minlength```: min length the string sent by the browser must match
* ```maxlength```: max length the string sent by the browser must match
...
* ```uiOptions.texts_are_vue_template```: treat `description` as Vue template
