...
describe('sub_and_defaults', function() {
    ...
    it('should handle if "value" then merge_patch_parent_properties', () => {
        const attrs = { duration: {
                optional: true,
                if: { value: "x2" },
                then: { merge_patch_parent_properties: { sn: {} } },
    ...
    it('should handle oneOf merge_patch_parent_properties', () => {
        const attrs = { duration: { oneOf: [ 
            { const: "", merge_patch_parent_properties: { sn: {} } }, 
            { const: "2" }
            ] } } as StepAttrsOption;

        test({ attrs, prev_defaults: { duration: "2" }, v: { duration: "" } as V },
             { attrNames: 'duration sn' });
    });
   ...
   it('should merge oneOf merge_patch_parent_properties and set merge_patch_options "ignore" later', () => {
        const attrs = { 
            profilename: { oneOf: [
                { const: "1" },
                { const: "2",
                    merge_patch_options: { newRootProperties: "ignore" },
                    merge_patch_parent_properties: { sn: { uiHidden: false } } },
                ] },
            _foo: { oneOf: [
                { const: "foo1", merge_patch_parent_properties: { sn: { uiHidden: true } } },
            ] },
        } as StepAttrsOption;

        let params = { attrs, v: { profilename: "2", _foo: "foo1" } as V }
        test(params, { subAttrs: { sn: { uiHidden: false } } });
        params.v["profilename"] = "1";
        test(params, { subAttrs: { sn: { uiHidden: true } } });
    });
