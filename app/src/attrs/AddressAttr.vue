...
<div v-else>
  <my-bootstrap-form-group name="country" :opts="opts" :validity="validity">
     <typeahead id="country" name="country" autocomplete="country-name" v-model="country" :options="conf.countries" v-magic-aria placeholder="Pays" :pattern="conf.pattern.country" :validity.sync="validity.country" :required="!opts.optional"></typeahead>
 ...
 <div v-if="country.toUpperCase() === 'FRANCE'">
  <my-bootstrap-form-group name="address_lines" :validity="validity">
    <input-with-validity name="address_lines" autocomplete="address-line1" v-model="lines" v-magic-aria placeholder="Numéro, rue" :validity.sync="validity.address_lines" :required="!opts.optional"></input-with-validity>
  ...
  <my-bootstrap-form-group name="postalCode">
   <div class="row">
    <div class="col-xs-3" :class="{'my-has-error': !validity.postalCode.valid}">
      <div>
        <input-with-validity name="postalCode" autocomplete="postal-code" v-model="postalCode" pattern="[0-9]{5}" v-magic-aria placeholder="Code postal" :required="!opts.optional" :validity.sync="validity.postalCode"></input-with-validity>
    ...
    <div class="col-xs-9" :class="{'my-has-error': !validity.town.valid}">
      <div>
        <typeahead name="town" autocomplete="address-level2" v-model="town" :options="towns" placeholder="Ville" :editable="true" :required="!opts.optional" :validity.sync="validity.town"></typeahead>
